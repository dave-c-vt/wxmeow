...
def test_issue_1_precipitation_display():
    ...
    try:
        ...
        print(f"✅ Processed {len(weather_obj.meowhourly)} hourly data points")
        ...
        if len(precip_data) > 0:
            ...
            print("✅ Issue 1 FIXED: Both temperature and precipitation data available")
        else:
            print("⚠️  No precipitation data found (may be normal for clear weather)")

        return True

    except Exception as e:
        print(f"❌ Issue 1 FAILED: {str(e)}")
        return False
...
def test_issue_2_day_filtering():
    ...
    for day_index in range(5):
        ...
        if actual_count == expected_count:
            print(f"   ✅ Day {day_index}: {actual_count} data points (correct)")
            successful_days += 1
        else:
            print(
                f"   ❌ Day {day_index}: {actual_count} data points (expected {expected_count})"
            )

    if successful_days == 5:
        print("✅ Issue 2 FIXED: All days have correct data filtering")
        return True
    else:
        print(f"❌ Issue 2 FAILED: Only {successful_days}/5 days working correctly")
        return False
...
def test_issue_3_temperature_alignment():
    ...
    for cell in temperature_cells:
        ...
        if temp == "??":
            print(f"   ⚠️  Day {day}: Missing temperature data")
            alignment_issues += 1
        elif alignment != "center":
            print(f"   ❌ Day {day}: Incorrect alignment '{alignment}'")
            alignment_issues += 1
        else:
            print(f"   ✅ Day {day}: {temp}°F (properly aligned)")
    ...
    if temp_cells == weather_icon_cells and alignment_issues == 0:
        print("✅ Issue 3 FIXED: Temperature alignment structure is correct")
        return True
    else:
        print(f"❌ Issue 3 FAILED: {alignment_issues} alignment issues found")
        return False
...
def test_chart_javascript_generation():
    ...
    try:
        ...
        print(f"✅ JSON serialization successful: {len(hourly_data)} items")
        ...
        if not missing_fields:
            print("✅ All required fields present in chart data")
            return True
        else:
            print(f"❌ Missing fields in chart data: {missing_fields}")
            return False
...
def simulate_chart_interaction():
    ...
    try:
        ...
        print(f"   ✅ Mock data created: {len(mock_data)} periods")
        steps_passed += 1
    except Exception as e:
    ...
    try:
        ...
        print(f"   ✅ Created {len(chart_containers)} chart containers")
        print(f"   ✅ Container 0 visible: {chart_containers[0]['display'] == 'block'}")
        steps_passed += 1
    except Exception as e:
    ...
    try:
        for day in [1, 2, 0]:  # Simulate clicking different days
            ...
            if (
                ...
                print(f"   ✅ Day {day} selection: correct container visible")
            else:
                print(f"   ❌ Day {day} selection: incorrect container visibility")
                raise Exception(f"Day {day} selection failed")

        steps_passed += 1
    except Exception as e:
        print(f"   ❌ Day selection failed: {str(e)}")
    ...
    try:
        ...
        if len(chart_instances) == 5:
            print("   ✅ All chart instances created")
            steps_passed += 1
        else:
            print(f"   ❌ Only {len(chart_instances)}/5 chart instances created")
    except Exception as e:
        print(f"   ❌ Data persistence failed: {str(e)}")

    # Final result
    if steps_passed == total_steps:
        print("✅ WORKFLOW TEST PASSED: All chart interaction steps successful")
        return True
    else:
        print(
            f"❌ WORKFLOW TEST FAILED: Only {steps_passed}/{total_steps} steps passed"
        )
        return False
