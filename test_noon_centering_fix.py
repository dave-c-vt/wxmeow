...
def test_noon_centering():
    ...
    if not os.path.exists(chart_js_path):
        print("❌ Chart JavaScript file not found!")
        return False
    ...
    if "min: -0.5" in js_content and "max: 23.5" in js_content:
        print("✅ Axis range configured for noon centering (-0.5 to 23.5)")
        tests_passed += 1
        ...
        if distance_from_center <= 0.5:
            print("   ✅ Noon is mathematically centered (within 0.5 units)")
            tests_passed += 1
        else:
            print("   ❌ Noon is not properly centered")

    elif "min: 0" in js_content and "max: 23" in js_content:
        print("⚠️  Original axis range detected (0 to 23) - noon not centered")
        print("   📐 With range 0-23, center is at 11.5, noon is at 12 (+0.5 offset)")
    else:
        print("❌ Could not determine axis range configuration")

    # Test 2: Check for noon emphasis with sun emoji
    if "🌞 12 PM" in js_content:
        print("✅ Sun emoji emphasis for noon implemented")
        tests_passed += 1
    else:
        print("❌ Sun emoji emphasis for noon missing")
    ...
    if grid_found:
        print("✅ Special grid line formatting for noon found")
        tests_passed += 1
    else:
        print("❌ Special grid line formatting for noon missing")

    # Test 4: Check for proper timezone handling
    if "ensureFullDayCoverage" in js_content:
        print("✅ Full day coverage function present")
        tests_passed += 1
    else:
        print("❌ Full day coverage function missing")

    # Test 5: Check for data sorting
    if "sort((a, b)" in js_content and "getHours()" in js_content:
        print("✅ Hour-based data sorting implemented")
        tests_passed += 1
    else:
        print("❌ Hour-based data sorting missing")
    ...
    if tests_passed >= 5:
        ...
        print("✅ The chart should now display with noon properly centered.")
        print("\n💡 To visually verify:")
